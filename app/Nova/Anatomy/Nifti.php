...
class Nifti extends Resource
{
     ...
     *
     * @var class-string<\App\Models\NiftiViewer>
     */
    public static $model = \App\Models\Nifti::class;
    ...
    public function fields(NovaRequest $request)
    {
        return [
            ID::make()->sortable(),
        Text::make('Directory Name')->rules('required', 'string')->onlyOnDetail(),
            Text::make('URL', 'urlCode')->onlyOnIndex()->resolveUsing(function () {
            // Assuming 'urlCode' is the field that stores the URL
            return $this->urlCode;
        }),
            Text::make('URL Code', 'urlCode')->onlyOnDetail(),
        
    File::make('Nifti Files', 'nifti')
        ->disk('public')
        ->store(function (Request $request, $model) {
            $directory = 'n';
            $originalFilename = $request->file('nifti')->getClientOriginalName();
            $path = $request->file('nifti')->storeAs($directory, $originalFilename, 'public');
